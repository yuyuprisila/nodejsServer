  ...
  const createTableQueries = [
    ...
    `
      CREATE TABLE IF NOT EXISTS tamu (
        id_tamu INT NULL AUTO_INCREMENT PRIMARY KEY,
        id_dataTamu INT(20),
...
//         INSERT INTO reservasi (id_employee, reservation_date, keterangan, jmlh_tamu, lokasi, ruangan, status, update_at, created_at)
//         VALUES (?, ?, ?, ?, ?, ?, 'WAITING', NOW(), NOW())`;

//       const insertReservasiValues = [id_employee,reservation_date, keterangan, jmlh_tamu, lokasi, ruangan];
...
app.post('/posts/create', (req, res) => {
  ...
  db.beginTransaction(err => {
    ...
    db.query(insertTamuQuery, insertTamuValues, (err, result) => {
      ...
      const insertReservasiQuery = `
        INSERT INTO reservasi (id_tamu, id_employee, reservation_date, keterangan, jmlh_tamu, lokasi, ruangan, status, create_at, update_at)
        VALUES (?, ?, ?, ?, ?, ?, ?, 'WAITING', NOW(), NOW())`;
